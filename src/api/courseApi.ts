...
const getCourseStudents = async (courseId: string): Promise<User[] | Error> => {
  try {
    const response = await axiosAuthorized.get(
      `/tutor/get/course/students/${courseId}`
    );
...
const getStudentCourses = async (): Promise<Course[] | Error> => {
  try {
    const response = await axiosAuthorized.get(`/get/student/courses`);
    const responseData = response.data as ResponseData;
...
const getCourseDetailsAdmin = async (): Promise<
  { _id: string; total: number }[] | []
> => {
  try {
    const response = await axiosAuthorized.get(
      "/admin/get/course/data/dashboard"
    );
...
const getCourseDetailsTutor = async (
  tutorId: string
): Promise<{ _id: string; total: number }[] | []> => {
  try {
    const response = await axiosAuthorized.get(
      `/get/tutor/course/data/dashboard/${tutorId}`
    );
    const data = response.data as ResponseData;
    return data.data as { _id: string; total: number }[];
  } catch (error) {
...
const getTutorPopularCourses = async (
  id: string
): Promise<Course[] | Error> => {
  try {
    const response = await axiosAuthorized.get(
      `/get/tutor/popular/courses/${id}`
    );
    const responseData = response.data as ResponseData;
    return responseData.courses as Course[];
  } catch (error) {
...
const removeStudentCoruse = async (
  courseId: string
): Promise<Course | Error> => {
  try {
    const response = await axiosAuthorized.patch(
      `/remove/student/course/${courseId}`
    );
    const { course } = response.data as ResponseData;
    return Promise.resolve(course as Course);
  } catch (error) {
